...
namespace json {
    ...
    DictItemContext Builder::StartDict()
    {
        if (step_stack_.empty()) 
            throw std::logic_error("Must start with builder"s);
        if (root_) 
    ...
    Builder& Builder::EndDict()
    {
        if (step_stack_.back() == Step::DICT) 
        {
            if (dicts_open_ > 0) 
            {
                ...
                throw std::logic_error("No dicts open"s);
        }
        else 
            throw std::logic_error("Cannot close dict"s);
    ...
    Builder& Builder::EndArray()
    {
        if (step_stack_.back() == Step::ARR) 
        {
            if (arrays_open_ > 0) 
            {
                ...
                throw std::logic_error("No arrays open"s);
        }
        else 
            throw std::logic_error("Cannot close array"s);
        ...
        if (step_stack_.back() == Step::DICT) 
        {
            if (dicts_open_ > 0) 
            {
                if (!keys_[dicts_open_ - 1]) 
                {
                    ...
                    throw std::logic_error("Key already set"s);
            }
            else 
                throw std::logic_error("No dicts open"s);
        }
        else 
            throw std::logic_error("Must start a dict"s);
        return *this;
    }
        ...
        else if (step_stack_.back() == Step::DICT) 
        {
            if (dicts_open_ > 0) 
            {
                if (keys_[dicts_open_ - 1]) 
                {
                    ...
                    throw std::logic_error("No key"s);
            }
            else 
                throw std::logic_error("No dicts open"s);
        }
    ...
    Node Builder::Build() const 
    {
        if (root_) 
        {
            ...
            throw std::logic_error("Must finish build"s);
    }
